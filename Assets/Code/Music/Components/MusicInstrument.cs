...
public class MusicInstrument : MonoBehaviour
{
    ...
    void PlayNote(Note note)
    {
        ...
        var volume = basevolume * note.volume;
    ...
    AudioSource GetFirstAvailableSource()
    {
        ...
        {
            AudioSource srcWithHighestTime = sources[0];
            for (int i = 0; i < sources.Count; i++)
            {
                ...
                else if (src.time > srcWithHighestTime.time)
                {
                    srcWithHighestTime = src;
                }
            }
            return srcWithHighestTime;
        }
    }
